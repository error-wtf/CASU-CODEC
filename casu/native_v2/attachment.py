...
def decode_attachment(payload: bytes) -> Attachment:
    ...
    try:
        ...
        if meta.get("version") != 1 or expected < 0 or expected > MAX_ATTACHMENT_BYTES:
            raise ValueError
        ...
        if filename != filename.replace("\\", "/").rsplit("/", 1)[-1] or filename in {"", ".", ".."}:
            raise ValueError
        ...
        if role_value is not None and (not isinstance(role_value, str)
            ...
            raise ValueError
        return Attachment(filename, str(meta["media_type"]), raw, digest,
                          role_value)
