...
class BitmapSubtitle:
    ...
    def canvas_rgba(self) -> np.ndarray:
        ...
        region = np.frombuffer(self.rgba, dtype=np.uint8).reshape(self.height,
                                                                  self.width, 4)
        canvas[self.y:self.y + self.height, self.x:self.x + self.width] = region
        return canvas


def encode_bitmap_subtitle(*, start_pts: int, end_pts: int,
                           canvas_width: int, canvas_height: int,
                           x: int, y: int, rgba: bytes,
                           width: int, height: int) -> bytes:
    raw = bytes(rgba)
    values = tuple(int(item) for item in (start_pts, end_pts, canvas_width,
                                           canvas_height, x, y, width, height))
    start, end, canvas_w, canvas_h, left, top, region_w, region_h = values
    expected = region_w * region_h * 4
    if (end < start or canvas_w <= 0 or canvas_h <= 0
            or canvas_w > MAX_BITMAP_DIMENSION or canvas_h > MAX_BITMAP_DIMENSION
            or canvas_w * canvas_h * 4 > MAX_BITMAP_RAW_BYTES
            or region_w <= 0 or region_h <= 0
            or left < 0 or top < 0 or left + region_w > canvas_w
            or top + region_h > canvas_h or expected != len(raw)
            or expected > MAX_BITMAP_RAW_BYTES):
        raise BitmapSubtitleError("invalid bitmap subtitle geometry/timing")
    compressed = zlib.compress(raw, 9)
    meta = {"version": 1, "start_pts": start, "end_pts": end,
            "time_base": [1, 1000], "canvas_width": canvas_w,
...
def decode_bitmap_subtitle(payload: bytes) -> BitmapSubtitle:
    ...
    try:
        ...
        if meta.get("time_base") != [1, 1000]:
            raise ValueError
        values = tuple(int(meta[key]) for key in (
            "start_pts", "end_pts", "canvas_width", "canvas_height",
            "x", "y", "width", "height", "raw_length", "compressed_length"))
        start, end, canvas_w, canvas_h, left, top, width, height, raw_length, compressed_length = values
        if (end < start or canvas_w <= 0 or canvas_h <= 0
                or canvas_w > MAX_BITMAP_DIMENSION or canvas_h > MAX_BITMAP_DIMENSION
                or canvas_w * canvas_h * 4 > MAX_BITMAP_RAW_BYTES
                or width <= 0 or height <= 0
                or left < 0 or top < 0 or left + width > canvas_w
                or top + height > canvas_h or raw_length != width * height * 4
                or raw_length > MAX_BITMAP_RAW_BYTES
                or compressed_length < 0
                or compressed_length > MAX_BITMAP_COMPRESSED_BYTES):
            raise ValueError
        ...
        if (len(raw) != raw_length or not inflater.eof or inflater.unused_data
