...
def _unpack(payload: bytes) -> tuple[dict, list[bytes]]:
    ...
    try:
        ...
        if (not isinstance(meta, dict) or not isinstance(meta.get("planes"), list)
                or not 0 < len(meta["planes"]) <= MAX_VIDEO_PLANES):
            raise ValueError
...
def encode_key_state(frame: CanonicalFrame) -> bytes:
    ...
    if (max(frame.shape) > MAX_VIDEO_DIMENSION
            or sum(int(plane.nbytes) for plane in frame.planes)
            > MAX_DECODED_PLANE_BYTES):
        raise VideoPayloadError("video key state exceeds safety limit")
    meta = _meta(frame)
    blobs = []
...
def encode_format_change(frame: CanonicalFrame) -> bytes:
    """Describe a new canonical video format; the next chunk must be a key."""
    value = {"version": 1, "pixel_format": frame.pixel_format,
...
def decode_format_change(payload: bytes) -> dict:
    ...
    try:
        ...
        if (not isinstance(shape, list) or len(shape) != 2
                or any(isinstance(item, bool) for item in shape)):
            raise ValueError
        height, width = (int(item) for item in shape)
        pixel_format = value.get("pixel_format")
        color = value.get("color_metadata", {})
        if (not 0 < width <= MAX_VIDEO_DIMENSION
                or not 0 < height <= MAX_VIDEO_DIMENSION
                or not isinstance(pixel_format, str) or not pixel_format
                or len(pixel_format) > 64 or not isinstance(color, dict)):
            raise ValueError
        return {"version": 1, "pixel_format": pixel_format,
                "source_shape": [height, width], "color_metadata": color}
...
def decode_key_state(payload: bytes) -> CanonicalFrame:
    ...
    for descriptor, compressed in zip(meta["planes"], blobs):
        ...
        if (max(shape) > MAX_VIDEO_DIMENSION
                or expected < 0 or total_decoded + expected > MAX_DECODED_PLANE_BYTES):
...
def encode_tile_update(frame: CanonicalFrame, *, x: int, y: int, width: int, height: int,
    ...
    if sum(int(part.nbytes) for part in parts) > MAX_DECODED_PLANE_BYTES:
        raise VideoPayloadError("video tile update exceeds safety limit")
...
class TileStateCache:
    ...
    def apply_tile_update(self, payload: bytes) -> CanonicalFrame:
        ...
        for index, (descriptor, compressed) in enumerate(zip(meta["planes"], blobs)):
            ...
            expected = int(np.prod(shape, dtype=np.int64)) * dtype.itemsize
            if (dtype.kind != "u" or dtype.itemsize not in (1, 2)
                    or expected != int(descriptor["raw_length"])
                    or max(shape) > MAX_VIDEO_DIMENSION
                    or total_decoded + expected > MAX_DECODED_PLANE_BYTES):
