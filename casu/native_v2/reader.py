...
from .validation import (NativeV2PayloadValidator,
                         NativeV2ValidationError, validate_manifest)
...
def _decode_recovery_point(payload: bytes, *, offset: int,
                           ...
                           allow_legacy_verified: bool = False) -> dict:
    try:
        ...
        if (value.get("version") != 1 or boundary not in prior_chunks
                or boundary >= offset):
            raise ValueError
        if declared_checkpoint is None or declared_prefix is None:
            ...
            checkpoint = json.dumps(value, sort_keys=True,
                                    separators=(",", ":"),
                                    allow_nan=False).encode("utf-8")
            if (str(declared_prefix) != prefix_sha256
                    or len(declared_checkpoint) != 64
                    or not hmac.compare_digest(
                        hashlib.sha256(checkpoint).hexdigest(),
                        declared_checkpoint)):
                raise ValueError
        for field_name, expected_type in (
                ("key_state_offsets", ChunkType.VIDEO_KEY_STATE),
                ("audio_block_offsets", ChunkType.AUDIO_BLOCK)):
            ...
            for raw_stream_id, raw_offset in entries.items():
                ...
                if (referenced is None or referenced.chunk_type != expected_type
                        or referenced.stream_id != int(raw_stream_id)
                        or int(raw_offset) > boundary):
                    raise ValueError
        if declared_checkpoint is not None:
            value["checkpoint_sha256"] = declared_checkpoint
        return value
    except (KeyError, TypeError, ValueError, json.JSONDecodeError,
            UnicodeDecodeError, RecursionError) as exc:
    ...
    def read_chunk_at(self, offset: int) -> tuple[NativeChunk, int]:
        ...
        with self.path.open("rb") as handle:
            ...
            if (payload_length > self.limits.max_chunk_bytes
                    ...
                    or uncompressed < payload_length
                    or uncompressed > self.limits.max_chunk_bytes
                    or flags != 0):
...
def read_native_v2(path: str | Path, *, max_manifest_bytes: int = 64 * 1024 * 1024,
                   ...
                   verify_payloads: bool | None = None,
                   limits: CasuLimits | None = None) -> NativeV2Container:
    source = Path(path)
    effective_limits = limits or replace(
        DEFAULT_LIMITS, max_manifest_bytes=max_manifest_bytes,
        max_chunk_bytes=max_chunk_bytes, max_chunks=max_chunks,
    ...
    with source.open("rb") as handle:
        ...
        if (manifest_length > effective_limits.max_manifest_bytes
                or manifest_length > size - HEADER.size):
        ...
        try:
            topology = NativeV2PayloadValidator(manifest, effective_limits,
                                                semantic=False)
        ...
        while handle.tell() < size:
            ...
            if (payload_length > effective_limits.max_chunk_bytes
                    or payload_length > size - handle.tell()
                    or uncompressed < payload_length
                    or uncompressed > effective_limits.max_chunk_bytes):
            ...
            if chunk_type == ChunkType.INTEGRITY_TABLE:
                ...
                observed_chunk_hashes.append(
                    (offset, hashlib.sha256(chunk_header + payload).hexdigest()))
            raw_chunk = NativeChunk(chunk_type, stream_id, pts, payload,
                                    flags, uncompressed)
            try:
            ...
            if chunk_type == ChunkType.SEEK_INDEX:
                ...
                try:
                    ...
                    if (len(integrity_expected) != 64
                            or any(character not in hexadecimal
                                   for character in integrity_expected)
                            or any(offset < HEADER.size or len(value) != 64
                ...
                recovery_points.append(_decode_recovery_point(
                    payload, offset=offset, prefix_sha256=digest_before_chunk,
                    prior_chunks=parsed_by_offset,
                    allow_legacy_verified=True))
    ...
    for entry in seek_entries:
        ...
        if (entry.target_pts != entry.key_state_pts
                or key is None or key.chunk_type != ChunkType.VIDEO_KEY_STATE
    ...
    if ({offset for offset, _value in chunk_hashes} != expected_offsets
            or len(dict(chunk_hashes)) != len(chunk_hashes)
            or dict(chunk_hashes) != dict(observed_chunk_hashes)):
        raise NativeV2Error("CASUNAT2 chunk hash table does not cover the verified prefix")
    indexed_keys = {(entry.stream_id, entry.key_state_offset)
                    for entry in seek_entries}
    actual_keys = {(chunk.stream_id, offset) for offset, chunk in zip(offsets, chunks)
    ...
    if verify_payloads:
        try:
            semantic = NativeV2PayloadValidator(manifest, effective_limits,
                                                semantic=True)
            for chunk in chunks:
            ...
            raise NativeV2Error(str(exc)) from exc
    return NativeV2Container(source, manifest, tuple(chunks), tuple(offsets),
                             seek_entries, verified, tuple(recovery_points),
                             chunk_hashes, effective_limits)
...
def recover_native_v2(path: str | Path, *, max_manifest_bytes: int = 64 * 1024 * 1024,
    ...
    try:
        ...
        raise NativeV2Error("damaged CASUNAT2 file is unavailable") from exc
    ...
    if manifest_length > max_manifest_bytes or HEADER.size + manifest_length > len(raw):
        raise NativeV2Error("invalid CASUNAT2 manifest length")
    try:
        manifest = strict_json_loads(raw[HEADER.size:HEADER.size + manifest_length])
    except StrictJsonError as exc:
        raise NativeV2Error("invalid CASUNAT2 manifest") from exc
    recovery_limits = replace(
        ...
        max_file_bytes=max_file_bytes)
    try:
        validate_manifest(manifest, recovery_limits)
    except NativeV2ValidationError as exc:
        raise NativeV2Error(str(exc)) from exc
    pos = HEADER.size + manifest_length
    chunks: list[NativeChunk] = []
    recovery: tuple[dict, int] | None = None
    prefix_digest = hashlib.sha256(raw[:pos])
    chunk_by_offset: dict[int, NativeChunk] = {}
    while pos + CHUNK_HEADER.size <= len(raw) and len(chunks) < max_chunks:
        ...
        if (flags != 0 or payload_length > max_chunk_bytes
                or payload_length > len(raw) - pos
                or uncompressed < payload_length or uncompressed > max_chunk_bytes):
            break
        ...
        if chunk_type == ChunkType.RECOVERY_POINT:
            try:
                value = _decode_recovery_point(
                    payload, offset=offset,
                    prefix_sha256=prefix_digest.hexdigest(),
                    prior_chunks=chunk_by_offset)
                recovery = (value, int(value["last_complete_chunk_offset"]))
            except NativeV2Error:
                break
        chunk_by_offset[offset] = chunk
        prefix_digest.update(raw[offset:pos])
    if recovery is None:
    ...
    usable = tuple(chunk for chunk, chunk_offset in zip(chunks, _chunk_offsets(raw, manifest_length))
                   ...
                   and chunk.chunk_type != ChunkType.RECOVERY_POINT)
