...
- SOURCES nav items exist (YOUTUBE / SPOTIFY / NETWORK STREAM)
...
- result activation resolves (mocked) and hands a direct URL to the player
...
def main() -> int:
    ...
    nav_texts = [b.text() for b in window._sidebar._nav_buttons]
    check("SOURCES nav items present",
          {"YOUTUBE", "SPOTIFY", "NETWORK STREAM"} <= set(nav_texts))
    ...
    def fake_open(source, display_label=None):
        window._show_player_page()
        opened.append((source, display_label))
    ...
    check("result activation resolved and opened (mocked)",
          opened and opened[0][0] == "https://www.youtube.com/watch?v=fake1"
          and opened[0][1] == "Result 1")
    check("player page restored after activation",
          window._center_stack.currentIndex() == 0)

    window._navigate("NETWORK STREAM")
    app.processEvents()
    check("NETWORK STREAM opens url mode in-window",
          window._center_stack.currentWidget() is window._sources_view
